...
def plot(ax):
    ...
    ###
    try:
        MSE_df = pd.read_hdf(os.path.join(DATA_DIR,'homogeneous_independent_gaussian_input_ESN/MSE_df.h5'), 'table')
    except:
        ...
        MSE_df.to_hdf(os.path.join(DATA_DIR,'homogeneous_independent_gaussian_input_ESN/MSE_df.h5'),'table')
    ###
    ...
    for simfile_inheterogeneous_independent_gaussian_input_ESNst in simfile:
        dat.append(np.load(simfile_inst))
    ...
    for dat_inst in dat:
        ...
        sigm_e = dat_inheterogeneous_independent_gaussian_input_ESNst['sigm_e']
        sigm_t = dat_inst['sigm_t']
        y = dat_inst['y']
        X_r = dat_heterogeneous_independent_gaussian_input_ESNinst['X_r']
        W = dat_inst['W']
...
if __name__ == '__main__':
    ...
    fig.savefig(os.path.join(PLOT_DIR,'homogeneous_independent_gaussian_input_rec_mem_pot_predict.pdf'))
    fig.savefig(os.path.join(PLOT_DIR,'homogeneous_independent_gaussian_input_rec_mem_pot_predict.png'),dpi=1000)
