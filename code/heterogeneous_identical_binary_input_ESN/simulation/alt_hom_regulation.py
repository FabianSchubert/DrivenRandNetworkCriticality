...
for k in tqdm(range(n_samples)):
    ...
    a = np.ones((N))*np.random.rand()*a_init_span
    b = np.zeros((N))
...
np.savez(os.path.join(DATA_DIR,'heterogeneous_identical_binary_input_ESN/alt_hom_regulation_'+str(datetime.now().isoformat())+'.npz'),
        a=a_rec,
        b=b_rec,
        W=W,
        y_norm=y_norm_rec,
