...
def plot(ax):
    ...
    for size in sizes:
        filelist.append(glob.glob(os.path.join(DATA_DIR,'heterogeneous_independent_gaussian_input_ESN/N_'+str(size) + '/param_sweep_*')))
...
if __name__ == '__main__':
    ...
    fig.savefig(os.path.join(PLOT_DIR,'heterogeneous_independent_gaussian_input_rec_mem_pot_predict_size_scaling.pdf'))
    fig.savefig(os.path.join(PLOT_DIR,'heterogeneous_independent_gaussian_input_rec_mem_pot_predict_size_scaling.png'),dpi=1000)
