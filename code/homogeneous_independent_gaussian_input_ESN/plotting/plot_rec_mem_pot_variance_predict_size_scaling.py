        ...
        for dat_inst in dat:
            ...
            for k in range(n_sigm_e):
                for l in range(n_sigm_t):
                    MSE = ((sigm_X_r[k,l,:]-sigm_w[k,l,:]*sigm_y[k,l,:]*N**.5)**2.).mean()*N
...
if __name__ == '__main__':
    ...
    fig.savefig(os.path.join(PLOT_DIR,'homogeneous_independent_gaussian_input_rec_mem_pot_predict_size_scaling.pdf'))
    fig.savefig(os.path.join(PLOT_DIR,'homogeneous_independent_gaussian_input_rec_mem_pot_predict_size_scaling.png'),dpi=1000)
