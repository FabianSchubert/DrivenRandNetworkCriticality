...
dx\tanh(ax) N_{\mu,\sigma}(x),
\qquad\quad
\sigma^2=\sigma_w^2\sigma_t^2+\sigma_{ext}^2\,,
...
\begin{equation}
\sigma_t^2=\int_{-\infty}^{\infty}
dz\tanh(z) \frac{1}{\sqrt{2\pi\sigma_a^2}}
\mathrm{e}^{-z^2/2\sigma_a^2}
\qquad\quad
\sigma_a^2=a^2\big(\sigma_w^2\sigma_t^2+\sigma_{ext}^2\big)\,,
\label{self_consistency_z}
\end{equation}
%
where the renormalized variance $\sigma_a^2$ can
be written as
%
\begin{equation}
\sigma_a^2=
a^2\sigma_w^2\big(\sigma_t^2+\sigma_{ext}^2/\sigma_w^2\big)\,.
\label{sigma_a_renormalized}
\end{equation}
%
The gain $a$ is adapted such that (\ref{self_consistency_z})
is fullfilled, which implies the scaling $a\sim 1/\sigma_w$.
The relation (\ref{sigma_a_renormalized}) shows furthermore
that the solution of (\ref{self_consistency_z}) depends
only on $\sigma_t$ and on the ratio $\sigma_{ext}/\sigma_w$,
which measures the relevance of the external driving with
respect the reccurent synaptic connections.

Approximating the transfer function with
$\tanh^2(z)\approx z^2-2 z^4/3$ one may carry out
the gaussian integral occuring on the right-hand
side of (\ref{self_consistency_z}), with the result
%
\begin{equation}
\sigma_t^2 +\frac{\sigma_{ext}^4}{\sigma_w^4}
+2\frac{\sigma_t^2\sigma_{ext}^2}{\sigma_w^2}
=\frac{\sigma_{ext}^2}{2\sigma_w^2},
\qquad\quad a=\frac{1}{\sigma_w}\,.
