...
parser.add_argument("--sigm_t",
help="target standard deviation of neural activity",
type=float,
default=0.561)
    ...
    for l in range(n_samples):
        ...
        if input_type == 0:
            ...
            adapt = rnn.run_hom_adapt(u_in=None,
                                    sigm_e=sigm_e,T=T_run_adapt,T_skip_rec=1000)

            #run sample after adaptation, USING THE INPUT STATISTICS OF THE ADAPTATION!!
            y_res,X_r_res,X_e_res = rnn.run_sample(u_in=None,sigm_e=sigm_e,T=T_run_sample+T_prerun_sample,show_progress=True)
