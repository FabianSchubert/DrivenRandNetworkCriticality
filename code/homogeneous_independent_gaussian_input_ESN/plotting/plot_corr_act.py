...
def plot(ax):
    ...
    try:
        corr_df = pd.read_hdf(os.path.join(DATA_DIR,'homogeneous_independent_gaussian_input_ESN/corr_df.h5'), 'table')
    except:
        ...
        corr_df.to_hdf(os.path.join(DATA_DIR,'homogeneous_independent_gaussian_input_ESN/corr_df.h5'),'table')
    ...
    if isinstance(file_search,list):
        ...
        timestamp = []
        for file_search_inst in file_search:
            simfile_in#check if there is already a saved dataframe...
    try:
        corr_df = pd.read_hdf(os.path.join(DATA_DIR,'heterogeneous_independent_gaussian_input_ESN/corr_df.h5'), 'table')
    except:
        print("No dataframe found! Creating it...")

        file_search = glob.glob(os.path.join(DATA_DIR,'heterogeneous_independent_gaussian_input_ESN/N_500/param_sweep_*'))
        ...
        corr_df.to_hdf(os.path.join(DATA_DIR,'heterogeneous_independent_gaussian_input_ESN/corr_df.h5'),'table')st, timestamp_inst = get_simfile_prop(os.path.join(DATA_DIR,file_search_inst))
            simfile.append(simfile_inst)
            timestamp.append(timestamp_inst)
    else:
        simfile,timestamp = get_simfile_prop(os.path.join(DATA_DIR,file_search))
