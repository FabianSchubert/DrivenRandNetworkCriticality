...
for k in tqdm(range(n_sweep_sigm_e)):
    for l in tqdm(range(n_sweep_sigm_t)):
        ...
        adapt = rnn.run_hom_adapt(T=T_run_adapt,sigm_e=sigm_e[k],T_skip_rec=1000)

        y_res,X_r_res,X_e_res = rnn.run_sample(T=T_run_sample+T_prerun_sample,sigm_e=sigm_e[k])
...
if not(os.path.isdir(os.path.join(DATA_DIR,'homogeneous_independent_gaussian_input_ESN/N_' + str(N)))):
    os.makedirs(os.path.join(DATA_DIR,'homogeneous_independent_gaussian_input_ESN/N_' + str(N)))

np.savez(os.path.join(DATA_DIR,'homogeneous_independent_gaussian_input_ESN/N_' + str(N) + '/param_sweep_'+str(datetime.now().isoformat())+'.npz'),
        sigm_t=sigm_t,
        sigm_e=sigm_e,
        y=y,
        X_r=X_r,
