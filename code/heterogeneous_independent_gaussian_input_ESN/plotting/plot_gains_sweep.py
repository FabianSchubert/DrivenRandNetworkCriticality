...
def plot(ax):
    ...
    simfile,timestamp = get_simfile_prop(os.path.join(DATA_DIR,'heterogeneous_independent_gaussian_input_ESN/N_500/param_sweep_'))
    ...
    cachefile_list = glob.glob(os.path.join(DATA_DIR,'heterogeneous_independent_gaussian_input_ESN/gain_solutions_*'))
        ...
        for l in tqdm(range(n_sigm_t)):
            for k in tqdm(range(n_sigm_e)):
                if not(sigm_e[k]==0 and sigm_t[l]==0):
                    ...
                    a_pred[k,l,:] = 1.
        print('saving result...')
        np.save(os.path.join(DATA_DIR,'heterogeneous_independent_gaussian_input_ESN/gain_solutions_'+timestamp+'.npy'),a_pred)
    ####
